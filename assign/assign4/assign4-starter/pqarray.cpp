...
void PQArray::enqueue(DataPoint elem) {
    if(this->isFull())
        this->expand(2);
    int index = this->getPosition(elem);
    this->insertToArray(elem, index);
}

//int PQArray::getCapacity() const {
...
bool PQArray::isFull() const{
    return size() == this->_numAllocated;
}
 ...
 */
void PQArray::expand(int multiple){
    DataPoint* oldArray = this->_elements;
    int oldSize = this->_numAllocated;  // 扩容时等价于int arraySize = size();
    this->_numAllocated *= multiple;
    this->_elements = new DataPoint[_numAllocated]();
    if(NULL != this->_elements) {
        for(int i=0; i<oldSize; ++i) {
            this->_elements[i] = oldArray[i];
        }
        delete[] oldArray;
    } else {
        error("内存扩展失败!");
 ...
 * 返回插入队列index;
 */
int PQArray::getPosition(const DataPoint& elem) const{
    int start = 0;
    int end = size() - 1;  // 下标和元素个数是减一关系, 此处又因为忘了减一出错
    while(start <= end) {
        int mid = (start + end) / 2;
        if(this->_elements[mid].priority < elem.priority) {
            end = mid - 1;  // 倒序
        } else if(this->_elements[mid].priority > elem.priority){
            start = mid + 1;
        } else {
            return mid;  // 与mid元素优先级相同, 也可以return mid+1
        }
    }
    // 没有当前优先级, 插入位置是start位置
    return start;
}
 ...
 * 将元素插入队列指定位置;
 */
void PQArray::insertToArray(const DataPoint& elem, int index){
    for(int i=size(); i>index; --i) {
        // 此处i=size(), 1个元素下标为0, 所以下面是i和i-1(下标和元素个数是减一关系)
 ...
 * priority order. Report an error if problem is found.
 */
...
STUDENT_TEST("PQArray, test same priority") {
...
STUDENT_TEST("PQArray, test same priority v2") {
    for (int size = 5; size <= 8; size *= 5) {
        PQArray pq;
        // 先入队-2, 最后入队多个-3优先级
        pq.enqueue({"", -2 });
