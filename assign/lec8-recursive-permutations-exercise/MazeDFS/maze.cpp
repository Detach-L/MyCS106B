...
#ifdef dfs
bool solveMazeRec(Grid<bool> &maze, Stack<GridLocation>& path, GridLocation& mazeExit) {
    if(path.peek() == mazeExit)  // 边界条件
        return true;
    auto neighbers = generateValidMoves(maze, path.peek());
    for (const auto &pos : neighbers) {
        if(!gatherVisited(path).contains(pos)) {  // 不能走回头路
            path.push(pos);  // 从相邻点选择一步
            MazeGraphics::highlightPath(path, "GREEN", 100);
            if(solveMazeRec(maze, path, mazeExit))  // 递归深度探索, 有返回值一定要处理
                return true;
            // 上面的if判断, 不能直接变成return solveMazeRec(maze, path, mazeExit); 会永远跳过下面语句不执行
            path.pop();  // 回溯状态, 选择另一步继续探索
            MazeGraphics::highlightPath(path, "GREEN", 100);
        }
    }
    return false;
...
Stack<GridLocation> solveMaze(Grid<bool>& maze) {
    MazeGraphics::drawGrid(maze);  // 绘制方格地图
    GridLocation mazeExit = {maze.numRows() - 1, maze.numCols() - 1};  // 定义迷宫终点
    Stack<GridLocation> path = {{0, 0}};  // 初始路径
    MazeGraphics::highlightPath(path, "GREEN", 100);
    if (solveMazeRec(maze, path, mazeExit))  // 递归深度优先搜索, 找到路径返回true
        return path;
    else  // 未找到路径返回false
        return {};
}
...
PROVIDED_TEST("solveMaze on file 10x16") {
    Grid<bool> maze;
    readMazeFile("res/10x16.maze", maze);
