...
int subdivideCanvas(GWindow &window, double x, double y, double width, double height, int order) {
    int count = 0;
    // 用阶定义复杂度
    if (order == 0) {
        return 1;  // order==0不用分割, 当前只有这个矩阵
    }
    // 判断长短边
    if (width > height) {
        // 在长边取一个分割点
        ...
        window.drawLine(x + mid, y, x + mid, y + height);
        // 对两个小矩形继续分割，同时控制复杂度 order
        int sunCount1 = subdivideCanvas(window, x, y, mid, height, order - 1);
        int sunCount2 = subdivideCanvas(window, x + mid, y, width - mid, height, order - 1);
        count = count + sunCount1 + sunCount2;
    } else {
        // 在长边取一个分割点
        double mid = 0.618 * width;
        // 画线将大矩形分割为两个小矩形
        window.drawLine(x, y + mid, x + width, y + mid);
        // 对两个小矩形继续分割，同时控制复杂度 order
        int sunCount1 = subdivideCanvas(window, x, y, width, mid, order - 1);
        int sunCount2 = subdivideCanvas(window, x, y + mid, width, height - mid, order - 1);
        count = count + sunCount1 + sunCount2;
    }
    return count;
}
...
//    }
    ...
    string text = string("Order ") + integerToString(order);
    window.drawString(text, 5, window.getHeight() - 5);
    int count = subdivideCanvas(window, 0, 0, window.getWidth(), window.getHeight(), order);
    window.drawString(integerToString(count), 5, window.getHeight() - 20);
    return 0;
}
